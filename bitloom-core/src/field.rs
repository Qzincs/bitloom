use serde::{Deserialize, Serialize};
use std::fmt::Display;

#[derive(Serialize, Deserialize, Clone, PartialEq, Debug)]
pub struct EnumVariant {
    pub value: i128,
    pub name: Option<String>,
    pub description: Option<String>,
}
#[derive(Serialize, Deserialize, Clone, PartialEq, Debug)]
pub enum FieldKind {
    Fixed(i128),
    Enum(Vec<EnumVariant>),
    Range {
        min: i128,
        max: i128,
        is_signed: bool,
    },
    Expr(String), // rhai script to compute the value
    Input,        // data provided by user input
}

impl Display for FieldKind {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            FieldKind::Fixed(_) => write!(f, "Fixed"),
            FieldKind::Enum(_) => write!(f, "Enum"),
            FieldKind::Range { .. } => write!(f, "Range"),
            FieldKind::Expr(_) => write!(f, "Expression"),
            FieldKind::Input => write!(f, "Input"),
        }
    }
}

#[derive(Serialize, Deserialize, Clone, PartialEq, Debug)]
pub enum FieldLength {
    /// Fixed length in bits
    Fixed(u32),
    Variable,
}

impl Display for FieldLength {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            FieldLength::Fixed(len) => write!(f, "{}", len),
            FieldLength::Variable => write!(f, "Variable"),
        }
    }
}

#[derive(Serialize, Deserialize, Clone, PartialEq, Debug)]
pub struct FieldRule {
    pub id: String,
    pub name: Option<String>,
    pub kind: FieldKind,
    pub length: FieldLength,
    pub description: Option<String>,
}

impl FieldRule {
    pub fn new(id: &str, kind: FieldKind, length: FieldLength) -> Self {
        Self {
            id: id.to_string(),
            name: None,
            kind,
            length,
            description: None,
        }
    }
}

impl Default for FieldRule {
    fn default() -> Self {
        Self {
            id: "new_field".to_string(),
            name: None,
            kind: FieldKind::Fixed(0),
            length: FieldLength::Fixed(8),
            description: None,
        }
    }
}

#[derive(Debug, Clone)]
pub struct ResolvedField {
    pub field: FieldRule,
    pub defined_in: String,
}

/// An instance of a field in a protocol message
pub struct Field {
    pub rule_id: String,
    pub value: Vec<u8>,
    pub ignore_rules: bool,
}

impl Field {
    pub fn new(rule_id: &str, value: Vec<u8>, ignore_rules: bool) -> Self {
        Self {
            rule_id: rule_id.to_string(),
            value,
            ignore_rules,
        }
    }

    pub fn set_value(&mut self, value: Vec<u8>) {
        self.value = value;
    }

    pub fn ignore_rules(&mut self, ignore: bool) {
        self.ignore_rules = ignore;
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    #[test]
    fn test_field_rule_creation() {
        let default_field = FieldRule::default();
        assert_eq!(default_field.id, "new_field");
        assert_eq!(default_field.length, FieldLength::Fixed(8));

        let custom_field = FieldRule::new("version", FieldKind::Fixed(4), FieldLength::Fixed(8));
        assert_eq!(custom_field.id, "version");
        assert_eq!(custom_field.kind, FieldKind::Fixed(4));
    }
}
